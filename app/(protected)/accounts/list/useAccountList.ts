...
import { ACCOUNT_LIST, DELETE_ACCOUNT } from '@/app/graphql/Account';
...
const useAccountList = ({ arrAccountDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrAccountDTO: arrAccountDTO,
    ...
    breadcrumbsItems: [{ label: 'Accounts' }]
  });
  ...
  const [getAccountList] = useLazyQuery(ACCOUNT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getAccountList();
    if (!error && data?.getAccountList?.accounts) {
      arrAccountDTO = data.getAccountList.accounts.map((item: AccountDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getAccountList?.total_records) {
        total_records = data.getAccountList.total_records;
      }
    }
    setState({
      arrAccountDTO: arrAccountDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/accounts/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/accounts/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteAccount, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteAccount, getData, state.arrSelectedId, toggleDialog]
  );
