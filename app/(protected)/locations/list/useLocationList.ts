...
import { LOCATION_LIST, DELETE_LOCATION } from '@/app/graphql/Location';
...
const useLocationList = ({ arrLocationDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrLocationDTO: arrLocationDTO,
    ...
    breadcrumbsItems: [{ label: 'Locations' }]
  });
  ...
  const [getLocationList] = useLazyQuery(LOCATION_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getLocationList();
      if (!error && data?.getLocationList?.locations) {
        arrLocationDTO = data.getLocationList.locations.map((item: LocationDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        if (data?.getLocationList?.total_records) {
          total_records = data.getLocationList.total_records;
        }
      }
      setState({
        arrLocationDTO: arrLocationDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/locations/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/locations/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteLocation, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteLocation, getData, state.arrSelectedId, toggleDialog]
  );
