...
const ClientLocationList = ({ arrLocationDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useLocationList({ arrLocationDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/locations/view/' + params.row.id}>{params.row.location_name}</MyLink>,
        []
      )
    },
    {
...
export default memo(ClientLocationList, (prevProps, nextProps) => {
