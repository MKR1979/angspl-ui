...
import { GET_FEE_PLAN_REVIEW_LIST, DELETE_FEE_PLAN } from '@/app/graphql/ScheduleFee';
...
const useScheduleFeeList = ({ arrScheduleFeeDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    arrScheduleFeeDTO: arrScheduleFeeDTO,
    dtoScheduleFee: SCHEDULE_FEE,
    ...
    breadcrumbsItems: [{ label: 'Schedule Fee' }]
  });
  ...
  const [deleteFeePlan] = useMutation(DELETE_FEE_PLAN, {});
  ...
  const [getFeePlanReviewList] = useLazyQuery(GET_FEE_PLAN_REVIEW_LIST, {
    ...
    variables: {
      ...
      from_date: state.dtoScheduleFee.from_date,
      to_date: state.dtoScheduleFee.to_date,
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getFeePlanReviewList({
        variables: {
          from_date: state.dtoScheduleFee.from_date,
          to_date: state.dtoScheduleFee.to_date,
          course_id: state.dtoScheduleFee.course_id !== -1 ? state.dtoScheduleFee.course_id : null,
          user_id: state.dtoScheduleFee.learner_id !== -1 ? state.dtoScheduleFee.learner_id : null
        }
      });

      if (!error && data) {
        arrScheduleFeeDTO = data.getFeePlanReviewList.feePlans.map((item: ScheduleFeeDTO) => ({
          ...item,
          id: parseInt(item.id.toString()),
          start_date: getMappedStartFrom(item.start_date)
        }));
        total_records = data.getFeePlanReviewList.total_records;
      }
    ...
    state.dtoScheduleFee.from_date,
    state.dtoScheduleFee.to_date,
    state.dtoScheduleFee.learner_id,
    state.dtoScheduleFee.course_id
  ]);
  ...
  //   async (params) => {
  //     router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/schedule-fee/edit/` + params.row.id);
  //   },
  //   [router]
  // );
  ...
  //     router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/schedule-fee/edit/` + state.selectedRow);
  //   },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteFeePlan({
          variables: {
            ids: params
          }
        });
    ...
    [deleteFeePlan, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onFromDateChange = useCallback(
    async (value: any): Promise<void> => {
      setState({ dtoScheduleFee: { ...state.dtoScheduleFee, from_date: value } } as StateType);
    },
    [state.dtoScheduleFee]
  );
  ...
  const onToDateChange = useCallback(
    async (value: any): Promise<void> => {
      setState({ dtoScheduleFee: { ...state.dtoScheduleFee, to_date: value } } as StateType);
    },
    [state.dtoScheduleFee]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteFeePlan, getData, state.arrSelectedId, toggleDialog]
  );

  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/schedule-fee/add`);
    },
    [router]
  );
...
export default useScheduleFeeList;
