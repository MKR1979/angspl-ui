...
import { GridEventListener, GridFilterModel, GridSortModel, useGridApiRef, GridInitialState, GridRowSelectionModel } from '@mui/x-data-grid';
...
import { GET_FEE_HEAD_LIST, DELETE_FEE_HEAD } from '@/app/graphql/FeeHead';
...
const useFeeHeadList = ({ arrFeeHeadDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrFeeHeadDTO: arrFeeHeadDTO,
    ...
    breadcrumbsItems: [{ label: 'Fee heads' }]
  });
  ...
  const [getFeeHeadList] = useLazyQuery(GET_FEE_HEAD_LIST, {
  ...
  const [deleteFeeHead] = useMutation(DELETE_FEE_HEAD, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getFeeHeadList();
      if (!error && data?.getFeeHeadList?.feeHeads) {
        arrFeeHeadDTO = data.getFeeHeadList.feeHeads.map((item: FeeHeadDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        if (data?.getFeeHeadList?.total_records) {
          total_records = data.getFeeHeadList.total_records;
        }
      }
      setState({
        arrFeeHeadDTO: arrFeeHeadDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/fee-heads/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/fee-heads/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteFeeHead({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteFeeHead) {
          getData();
    ...
    [deleteFeeHead, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${Constants.ADMIN_PAYMENT_MODULES}/fee-heads/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteFeeHead, getData, state.arrSelectedId, toggleDialog]
  );
