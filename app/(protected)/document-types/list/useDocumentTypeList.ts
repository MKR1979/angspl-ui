...
import { DOCUMENT_TYPE_LIST, DELETE_DOCUMENT_TYPE } from '@/app/graphql/DocumentType';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrDocumentTypeDTO: DocumentTypeDTO[];
...
const useDocumentTypeList = ({ arrDocumentTypeDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrDocumentTypeDTO: arrDocumentTypeDTO,
    ...
    breadcrumbsItems: [{ label: 'Document Types' }]
  });
  ...
  const [getDocumentTypeList] = useLazyQuery(DOCUMENT_TYPE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getDocumentTypeList();
    if (!error && data?.getDocumentTypeList?.documentTypes) {
      arrDocumentTypeDTO = data.getDocumentTypeList.documentTypes.map((item: DocumentTypeDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getDocumentTypeList?.total_records) {
        total_records = data.getDocumentTypeList.total_records;
      }
    }
    setState({
      arrDocumentTypeDTO: arrDocumentTypeDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/document-types/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-types/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteDocumentType({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteDocumentType) {
          getData();
    ...
    [deleteDocumentType, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-types/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDocumentType, getData, state.arrSelectedId, toggleDialog]
  );
