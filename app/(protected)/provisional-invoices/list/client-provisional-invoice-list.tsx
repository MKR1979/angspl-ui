...
import useProvisionalInvoiceList from './useProvisionalInvoiceList';
...
const ClientProvisionalInvoiceList = ({ arrProvisionalInvoiceDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useProvisionalInvoiceList({ arrProvisionalInvoiceDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'provisional_invoice_no',
      headerName: 'Provisional Invoice #',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => (
          <MyLink href={'/provisional-invoices/view/' + params.row.id}>{params.row.provisional_invoice_no}</MyLink>
        ),
        []
      )
    },
    {
      field: 'provisional_invoice_date',
      headerName: 'Provisional Invoice Date',
      flex: 1,
  ...
  console.log('hello', state.arrProvisionalInvoiceDTO);
  return (
    <>
      ...
      <MyCard>
        <MyCardContent>
          <MyDataGrid
            ...
            rows={state.arrProvisionalInvoiceDTO}
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Provisional Invoice Removal"
...
export default memo(ClientProvisionalInvoiceList, (prevProps, nextProps) => {
