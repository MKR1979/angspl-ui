...
import { PROVISIONAL_INVOICE_LIST, DELETE_PROVISIONAL_INVOICE } from '@/app/graphql/ProvisionalInvoice';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrProvisionalInvoiceDTO: ProvisionalInvoiceDTO[];
...
const useProvisionalInvoiceList = ({ arrProvisionalInvoiceDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    breadcrumbsItems: [{ label: 'Provisional Invoices' }]
  });
  ...
  const [getProvisionalInvoiceList] = useLazyQuery(PROVISIONAL_INVOICE_LIST, {
  ...
  const [deleteProvisionalInvoice] = useMutation(DELETE_PROVISIONAL_INVOICE, {});

  const getData = useCallback(async (): Promise<void> => {
    setState({ isLoading: true } as StateType);
    let arrProvisionalInvoiceDTO: ProvisionalInvoiceDTO[] = [];
    let total_records: number = 0;
    const { error, data } = await getProvisionalInvoiceList();
    if (!error && data?.getProvisionalInvoiceList?.provisionalInvoices) {
      arrProvisionalInvoiceDTO = data.getProvisionalInvoiceList.provisionalInvoices.map((item: ProvisionalInvoiceDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getProvisionalInvoiceList?.total_records) {
        total_records = data.getProvisionalInvoiceList.total_records;
      }
    }
    setState({
      arrProvisionalInvoiceDTO: arrProvisionalInvoiceDTO,
  ...
  const handleContextMenu = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      ...
      setState({
        ...
        contextMenu:
          state.arrProvisionalInvoiceDTO.filter((item) => item.id === Number(event.currentTarget.getAttribute('data-id')))[0]?.status !=
            'Cancelled' && state.contextMenu === null
            ? { mouseX: event.clientX - 2, mouseY: event.clientY - 4 }
            : null
      } as StateType);
    },
    [state.contextMenu, state.arrProvisionalInvoiceDTO]
  );
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      if (params.row.status != 'Cancelled') {
        router.push('/provisional-invoices/edit/' + params.row.id);
      }
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/provisional-invoices/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteProvisionalInvoice({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteProvisionalInvoice) {
          getData();
    ...
    [deleteProvisionalInvoice, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/provisional-invoices/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteProvisionalInvoice, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useProvisionalInvoiceList;
