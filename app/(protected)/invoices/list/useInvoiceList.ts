...
import { INVOICE_LIST, DELETE_INVOICE } from '@/app/graphql/Invoice';
...
const useInvoiceList = ({ arrInvoiceDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrInvoiceDTO: arrInvoiceDTO,
    ...
    breadcrumbsItems: [{ label: 'Invoices' }]
  });
  ...
  const [getInvoiceList] = useLazyQuery(INVOICE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getInvoiceList();
    if (!error && data?.getInvoiceList?.invoices) {
      arrInvoiceDTO = data.getInvoiceList.invoices.map((item: InvoiceDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getInvoiceList?.total_records) {
        total_records = data.getInvoiceList.total_records;
      }
    }
    setState({
      arrInvoiceDTO: arrInvoiceDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/invoices/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/invoices/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteInvoice, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteInvoice, getData, state.arrSelectedId, toggleDialog]
  );
