...
import { OPTION_LIST, DELETE_OPTION } from '@/app/graphql/Option';
...
const useOptionList = ({ arrOptionDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrOptionDTO: arrOptionDTO,
    ...
    breadcrumbsItems: [{ label: 'Options' }]
  });
  ...
  const [getOptionList] = useLazyQuery(OPTION_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getOptionList();
      if (!error && data) {
        arrOptionDTO = data.getOptionList.options.map((item: OptionDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getOptionList.total_records;
      }
      setState({ arrOptionDTO: arrOptionDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
    } catch (err) {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push(`/options/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/options/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteOption, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteOption, getData, state.arrSelectedId, toggleDialog]
  );
