...
const ClientOptionList = ({ arrOptionDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useOptionList({ arrOptionDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 99) ? (
            <MyLink href={`/options/view/` + params.row.id}>{params.row.option_name}</MyLink>
          ) : (
            <span>{params.row.option_name}</span>
          ),
        [userPermissions]
      )
    },
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrOptionDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 96)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 97)}
          ...
          {(findPermission(userPermissions, 98) || findPermission(userPermissions, 97)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 98) && (
                <MyMenuItem onClick={onEditClick}>
                  <MyEditIcon />
                  Edit
                </MyMenuItem>
              )}
              {findPermission(userPermissions, 97) && (
...
export default memo(ClientOptionList, (prevProps, nextProps) => {
