...
type Props = {
  arrGroupDTO: GroupDTO[];
  total_records: number;
};
const ClientGroupList = ({ arrGroupDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useGroupList({arrGroupDTO,total_records });
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 174) ? (
            <MyLink href={'/groups/view/' + params.row.id}>{params.row.group_name}</MyLink>
          ) : (
...
export default memo(ClientGroupList, (prevProps, nextProps) => {
