...
import { GROUP_LIST, DELETE_GROUP } from '@/app/graphql/Group';
...
const useGroupList = ({ arrGroupDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrGroupDTO: arrGroupDTO,
  ...
  const [getGroupList] = useLazyQuery(GROUP_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getGroupList();
      if (!error && data) {
        arrGroupDTO = data.getGroupList.groups.map((item: GroupDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getGroupList.total_records;
      }
      setState({
        arrGroupDTO: arrGroupDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/groups/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/groups/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteGroup, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteGroup, getData, state.arrSelectedId, toggleDialog]
  );
