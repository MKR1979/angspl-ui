...
import { EVENT_LIST, DELETE_EVENT } from '@/app/graphql/Event';
...
const useEventList = ({ arrEventDTO, total_records }: Props) => {
  ...
  const [getEventList] = useLazyQuery(EVENT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      if (!error && data?.getEventList?.events) {
        arrEventDTO = data.getEventList.events.map((item: EventDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        if (data?.getEventList?.total_records) {
          total_records = data.getEventList.total_records;
        }
      }
      setState({
        arrEventDTO: arrEventDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/events/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteEvent, getData, state.arrSelectedId, toggleDialog]
  );
  ...
  const onCalendarViewClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>) => {
      event.preventDefault();
      router.push('/events/list/calendar');
    },
    [router]
  );
