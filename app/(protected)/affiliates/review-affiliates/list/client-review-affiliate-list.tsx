...
import useAffiliateList from './useReviewAffiliateList';
...
const ClientAffiliateList = ({ arrAffiliateDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onDeleteSingleClose,
    CalculatePayment
  } = useAffiliateList({ arrAffiliateDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 129) ? (
            <MyLink href={`/${Constants.ADMIN_AFFILIATE_DASHBOARD}/review-affiliates/view/` + params.row.id}>
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        {findPermission(userPermissions, 211) && (
          <MyButton
          ...
          <MyGrid container spacing={2} style={{ border: '3px solid rgb(238, 242, 246)', padding: '1rem' }}>
            <MyGrid size={{ xs: 12, sm: 6 }}>
              <MyAutocomplete
                ...
                onClose={setClose1}
                value={
                  state.dtoAffiliate.user_id === null
                    ? { id: 'ALL', text: 'All Users' }
                    : { id: state.dtoAffiliate.user_id, text: state.dtoAffiliate.user_name }
                }
            ...
            <MyGrid size={{ xs: 12, sm: 3 }}>
              <MyDatePicker
                ...
                value={
                  dayjs(state.dtoAffiliate.from_date).format('DD-MM-YYYY') === '12/31/1899'
                    ? null
                    : dayjs(state.dtoAffiliate.from_date).toDate()
                }
            ...
            <MyGrid size={{ xs: 12, sm: 3 }}>
              <MyDatePicker
                ...
                value={
                  dayjs(state.dtoAffiliate.to_date).format('DD-MM-YYYY') === '12/31/1899'
                    ? null
                    : dayjs(state.dtoAffiliate.to_date).toDate()
                }
                // maxDate={dayjs().toDate()}
                minDate={
                  state.dtoAffiliate.from_date && dayjs(state.dtoAffiliate.from_date).isValid()
                    ? dayjs(state.dtoAffiliate.from_date).toDate()
                    : undefined
                }
                maxDate={dayjs().toDate()}
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrAffiliateDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 126)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 127)}
          ...
          {(findPermission(userPermissions, 128) || findPermission(userPermissions, 127)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 128) && (
                <MyMenuItem onClick={onEditClick}>
                  <MyEditIcon />
                  Edit
                </MyMenuItem>
              )}
              {findPermission(userPermissions, 127) && (
...
export default memo(ClientAffiliateList, (prevProps, nextProps) => {
