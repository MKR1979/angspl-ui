...
import { AFFILIATE_REVIEW_LIST, CALCULATE_AFFILIATE_PAYMENT, DELETE_AFFILIATE } from '@/app/graphql/Affiliate';
...
const useAffiliateList = ({ arrAffiliateDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    open1: false,
    isLoading: false,
    arrAffiliateDTO: arrAffiliateDTO,
    ...
    breadcrumbsItems: [{ label: 'Review Affiliate' }]
  });
  ...
  const [calculateAffiliatePayment] = useMutation(CALCULATE_AFFILIATE_PAYMENT, {});
  const [getUserLookup] = useLazyQuery(USER_LOOKUP, { fetchPolicy: 'network-only' });

  const [getAffiliateReviewList] = useLazyQuery(AFFILIATE_REVIEW_LIST, {
    ...
    variables: {
      ...
      from_date: state.dtoAffiliate.from_date,
      to_date: state.dtoAffiliate.to_date,
      user_id: state.dtoAffiliate.user_id
    }
  });
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getAffiliateReviewList({
        variables: {
          from_date: state.dtoAffiliate.from_date,
          to_date: state.dtoAffiliate.to_date,
          user_id: state.dtoAffiliate.user_id !== -1 ? state.dtoAffiliate.user_id : null
        }
      });
      if (!error && data) {
        arrAffiliateDTO = data.getAffiliateReviewList.affiliates.map((item: AffiliateDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getAffiliateReviewList.total_records;
      }
      setState({
        arrAffiliateDTO: arrAffiliateDTO,
        total_records: total_records,
  ...
  }, [getAffiliateReviewList, state.dtoAffiliate.user_id, state.dtoAffiliate.from_date, state.dtoAffiliate.to_date]);
  ...
  const onFromDateChange = useCallback(
    async (value: any): Promise<void> => {
      setState({ dtoAffiliate: { ...state.dtoAffiliate, from_date: value } } as StateType);
    },
    [state.dtoAffiliate]
  );
  ...
  const onToDateChange = useCallback(
    async (value: any): Promise<void> => {
      setState({ dtoAffiliate: { ...state.dtoAffiliate, to_date: value } } as StateType);
    },
    [state.dtoAffiliate]
  );
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (params) => {
      ...
      router.push(`/${Constants.ADMIN_AFFILIATE_DASHBOARD}/review-affiliates/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      dispatch(setIsEditMode(true));
      router.push(`/${Constants.ADMIN_AFFILIATE_DASHBOARD}/review-affiliates/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteAffiliate, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      ...
      router.push(`/${Constants.ADMIN_AFFILIATE_DASHBOARD}/review-affiliates/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteAffiliate, getData, state.arrSelectedId, toggleDialog]
  );

  const CalculatePayment = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      const firstId = Array.isArray(state.arrSelectedId) ? parseInt(state.arrSelectedId[0]) : parseInt(state.arrSelectedId);
      const { data } = await calculateAffiliatePayment({
        variables: {
          affiliate_id: firstId
        }
      });
      if (data) {
        getData();
        showSnackbar(gMessageConstants.SNACKBAR_PAYMENT_CAL_SUCCESS, 'success');
      } else {
        showSnackbar(gMessageConstants.SNACKBAR_PAYMENT_CAL_FAIL, 'error');
      }
    },
    [calculateAffiliatePayment, getData, state.arrSelectedId, toggleDialog]
  );
