...
import { TAX_LIST, DELETE_TAX } from '@/app/graphql/Tax';
...
const useTaxList = ({ arrTaxDTO, total_records }: Props) => {
  ...
  const [getTaxList] = useLazyQuery(TAX_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getTaxList();
    if (!error && data?.getTaxList?.taxes) {
      arrTaxDTO = data.getTaxList.taxes.map((item: TaxDTO) => {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/taxes/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/taxes/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteTax, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteTax, getData, state.arrSelectedId, toggleDialog]
  );
