...
const ClientTaxList = ({ arrTaxDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useTaxList({ arrTaxDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/taxes/view/' + params.row.id}>{params.row.tax_description}</MyLink>,
        []
      )
    },
    {
...
export default memo(ClientTaxList, (prevProps, nextProps) => {
