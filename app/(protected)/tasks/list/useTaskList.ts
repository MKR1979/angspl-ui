...
import { TASK_LIST, DELETE_TASK } from '@/app/graphql/Task';
...
const useTaskList = ({ arrTaskDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrTaskDTO: arrTaskDTO,
  ...
  const [getTaskList] = useLazyQuery(TASK_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getTaskList();
    if (!error && data?.getTaskList?.tasks) {
      arrTaskDTO = data.getTaskList.tasks.map((item: TaskDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getTaskList?.total_records) {
        total_records = data.getTaskList.total_records;
      }
    }
    setState({
      arrTaskDTO: arrTaskDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/tasks/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/tasks/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteTask, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteTask, getData, state.arrSelectedId, toggleDialog]
  );
