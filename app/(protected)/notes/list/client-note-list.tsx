...
const ClientNoteList = ({ arrNoteDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useNoteList({ arrNoteDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/notes/view/' + params.row.id}>{params.row.subject}</MyLink>,
        []
      )
    },
    {
  ...
  console.log('hello', state.arrNoteDTO);
  return (
    <>
...
export default memo(ClientNoteList, (prevProps, nextProps) => {
