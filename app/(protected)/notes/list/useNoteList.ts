...
import { NOTE_LIST, DELETE_NOTE } from '@/app/graphql/Note';
...
const useNoteList = ({ arrNoteDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrNoteDTO: arrNoteDTO,
  ...
  const [getNoteList] = useLazyQuery(NOTE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getNoteList();
    if (!error && data?.getNoteList?.notes) {
      arrNoteDTO = data.getNoteList.notes.map((item: NoteDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getNoteList?.total_records) {
        total_records = data.getNoteList.total_records;
      }
    }
    setState({
      arrNoteDTO: arrNoteDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/notes/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/notes/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteNote, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteNote, getData, state.arrSelectedId, toggleDialog]
  );
