...
const ClientStageList = ({ arrStageDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useStageList({ arrStageDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/stages/view/' + params.row.id}>{params.row.stage_name}</MyLink>,
        []
      )
    }
  ];
  console.log('hello', state.arrStageDTO);
  return (
    <>
...
export default memo(ClientStageList, (prevProps, nextProps) => {
