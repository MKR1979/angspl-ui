...
import { STAGE_LIST, DELETE_STAGE } from '@/app/graphql/Stage';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrStageDTO: StageDTO[];
...
const useStageList = ({ arrStageDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrStageDTO: arrStageDTO,
  ...
  const [getStageList] = useLazyQuery(STAGE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getStageList();
    if (!error && data?.getStageList?.stages) {
      arrStageDTO = data.getStageList.stages.map((item: StageDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getStageList?.total_records) {
        total_records = data.getStageList.total_records;
      }
    }
    setState({
      arrStageDTO: arrStageDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/stages/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/stages/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteStage, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteStage, getData, state.arrSelectedId, toggleDialog]
  );
