...
import { EMAIL_TEMPLATE_LIST, DELETE_EMAIL_TEMPLATE } from '@/app/graphql/EmailTemplate';
...
type StateType = {
  isLoading: boolean;
  arrEmailTemplateDTO: EmailTemplateDTO[];
...
const useEmailTemplateList = ({ arrEmailTemplateDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrEmailTemplateDTO: arrEmailTemplateDTO,
    ...
    breadcrumbsItems: [{ label: 'Email Templates' }]
  });
  ...
  const [getEmailTemplateList] = useLazyQuery(EMAIL_TEMPLATE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getEmailTemplateList();
      if (!error && data) {
        arrEmailTemplateDTO = data.getEmailTemplateList.email_templates.map((item: EmailTemplateDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getEmailTemplateList.total_records;
      }
      setState({
        arrEmailTemplateDTO: arrEmailTemplateDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/email-templates/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/email-templates/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteEmailTemplate, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/email-templates/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteEmailTemplate, getData, state.arrSelectedId, toggleDialog]
  );
