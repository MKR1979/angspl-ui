...
const ClientEmailTemplateList = ({ arrEmailTemplateDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useEmailTemplateList({arrEmailTemplateDTO,total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 174) ? (
            <MyLink href={'/email-templates/view/' + params.row.id}>{params.row.email_template_name}</MyLink>
          ) : (
            <span>{params.row.email_template_name}</span>
          ),
        [userPermissions]
      )
    },
    ...
    <>
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrEmailTemplateDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 171)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 172)}
          ...
          {(findPermission(userPermissions, 173) || findPermission(userPermissions, 172)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 173) && (
                <MyMenuItem onClick={onEditClick}>
                  <MyEditIcon />
                  Edit
                </MyMenuItem>
              )}
              {findPermission(userPermissions, 172) && (
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Email Template Removal"
...
export default memo(ClientEmailTemplateList, (prevProps, nextProps) => {
