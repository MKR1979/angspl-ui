...
import { DOCUMENT_LIST, DELETE_DOCUMENT } from '@/app/graphql/Document';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrDocumentDTO: DocumentDTO[];
...
const useDocumentList = ({ arrDocumentDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrDocumentDTO: arrDocumentDTO,
    ...
    breadcrumbsItems: [{ label: 'Documents' }]
  });
  ...
  const [getDocumentList] = useLazyQuery(DOCUMENT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getDocumentList();
    if (!error && data?.getDocumentList?.documents) {
      arrDocumentDTO = data.getDocumentList.documents.map((item: DocumentDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getDocumentList?.total_records) {
        total_records = data.getDocumentList.total_records;
      }
    }
    setState({
      arrDocumentDTO: arrDocumentDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/documents/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/documents/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteDocument, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDocument, getData, state.arrSelectedId, toggleDialog]
  );
