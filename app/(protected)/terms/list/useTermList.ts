...
import { TERM_LIST, DELETE_TERM } from '@/app/graphql/Term';
...
const useTermList = ({ arrTermDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrTermDTO: arrTermDTO,
  ...
  const [getTermList] = useLazyQuery(TERM_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getTermList();
    if (!error && data?.getTermList?.terms) {
      arrTermDTO = data.getTermList.terms.map((item: TermDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getTermList?.total_records) {
        total_records = data.getTermList.total_records;
      }
    }
    setState({
      arrTermDTO: arrTermDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/terms/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/terms/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteTerm, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteTerm, getData, state.arrSelectedId, toggleDialog]
  );
