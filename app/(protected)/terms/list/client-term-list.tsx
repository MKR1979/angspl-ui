...
import useTermList from './useTermList';
...
const ClientTermList = ({ arrTermDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useTermList({ arrTermDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/terms/view/' + params.row.id}>{params.row.term_code}</MyLink>,
        []
      )
    },
    {
...
export default memo(ClientTermList, (prevProps, nextProps) => {
