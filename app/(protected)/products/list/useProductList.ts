...
import { PRODUCT_LIST, DELETE_PRODUCT } from '@/app/graphql/Product';
...
const useProductList = ({ arrProductDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrProductDTO: arrProductDTO,
    ...
    breadcrumbsItems: [{ label: 'Products' }]
  });
  ...
  const [getProductList] = useLazyQuery(PRODUCT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getProductList();
    if (!error && data?.getProductList?.products) {
      arrProductDTO = data.getProductList.products.map((item: ProductDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getProductList?.total_records) {
        total_records = data.getProductList.total_records;
      }
    }
    setState({ arrProductDTO: arrProductDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
  }, [getProductList]);
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/products/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteProduct, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteProduct, getData, state.arrSelectedId, toggleDialog]
  );
