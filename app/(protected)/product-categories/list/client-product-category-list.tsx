...
const ClientProductCategoryList = ({ arrProductCategoryDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useProductCategoryList({ arrProductCategoryDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => (
          <MyLink href={'/product-categories/view/' + params.row.id}>{params.row.product_category_name}</MyLink>
        ),
        []
      )
    }
    ...
    <>
      ...
      <MyCard>
        <MyCardContent>
          <MyDataGrid
            ...
            rows={state.arrProductCategoryDTO}
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Product Category Removal"
...
export default memo(ClientProductCategoryList, (prevProps, nextProps) => {
