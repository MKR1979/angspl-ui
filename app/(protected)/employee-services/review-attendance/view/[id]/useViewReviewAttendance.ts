...
const useViewAttendance = ({ dtoAttendance }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    breadcrumbsItems: [{ label: 'Review Attendance', href: `/${Constants.ADMIN_EMP_DASHBOARD}/review-attendance/list` }, { label: 'View Attendance' }]
  });
...
const showSnackbar = useSnackbar();
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>) => {
      ...
      router.push(`/${Constants.ADMIN_EMP_DASHBOARD}/review-attendance/edit/` + state.dtoAttendance.id);
    },
    [router, state.dtoAttendance.id]
  );
