...
import useTrackPresenceList from './useTrackPresenceList';
...
const formatTo12HourWithSeconds = (time24: string | null | undefined): string => {
  if (!time24 || !/^\d{2}:\d{2}:\d{2}$/.test(time24)) {
    return ''; // return blank if empty or invalid
  }
  const [hours, minutes, seconds] = time24.split(':');
  const date = new Date();
  date.setHours(parseInt(hours), parseInt(minutes), parseInt(seconds));
  if (isNaN(date.getTime())) {
    return ''; // also blank on invalid date
  }
...
const ClientTrackPresenceList = ({ arrTrackPresenceDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    onCheckChange,
    ...
    onToDateChange,
    onReportTypeChange
  } = useTrackPresenceList({ arrTrackPresenceDTO, total_records });
          ...
          <MyGrid container spacing={2} style={{ border: '3px solid rgb(238, 242, 246)', padding: '1rem' }}>
            ...
            <MyGrid size={{ xs: 12, sm: 2.5 }}>
              <MyDatePicker
                ...
                value={
                  dayjs(state.dtoTrackPresence.to_date).format('yyyy-mm-dd') === '12/31/1899'
                    ? null
                    : dayjs(state.dtoTrackPresence.to_date).toDate()
                }
                maxDate={dayjs().toDate()}
                minDate={dayjs(gConstants.MIN_ALLOWED_DATE_DPS).toDate()}
              />
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          ...
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              showExportButton={findPermission(userPermissions, 203)}
...
export default memo(ClientTrackPresenceList, (prevProps, nextProps) => {
