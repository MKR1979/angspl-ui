...
const useTrackPresenceList = ({ arrTrackPresenceDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    initialState: {
      pagination: {
        paginationModel: { pageSize: 100, page: 0 } // Show all records on one page
      },
      columns: {
    ...
    try {
      ...
      const { error, data } = await getPresenceSummary({
        variables: {
          ...
          to_date: state.dtoTrackPresence.to_date,
  ...
  const onToDateChange = useCallback(
    async (value: any): Promise<void> => {
      setState({ dtoTrackPresence: { ...state.dtoTrackPresence, to_date: value } } as StateType);
    },
    [state.dtoTrackPresence]
  );
...
export default useTrackPresenceList;
