...
import useCompanyList from './useCompanyDomainList';
...
const ClientCompanyDomainList = ({ arrCompanyDomainDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useCompanyList({ arrCompanyDomainDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 44) ? (
            <MyLink href={'/company-domains/view/' + params.row.id}>{params.row.company_name}</MyLink>
          ) : (
    ...
    {
      field: 'logo_height',
      headerName: 'Logo Height',
      flex: 1,
      minWidth: 150
    },
    {
      field: 'logo_width',
      headerName: 'Logo Width',
      flex: 1,
      minWidth: 150
    },
    {
    ...
    <>
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrCompanyDomainDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 41)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 42)}
          ...
          {(findPermission(userPermissions, 43) || findPermission(userPermissions, 42)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 43) && (
                <MyMenuItem onClick={onEditClick}>
                  <MyEditIcon />
                  Edit
                </MyMenuItem>
              )}
              {findPermission(userPermissions, 42) && (
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Company Domain Removal"
...
export default memo(ClientCompanyDomainList, (prevProps, nextProps) => {
