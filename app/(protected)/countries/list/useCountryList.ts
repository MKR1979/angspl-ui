...
const useCountryList = ({ arrCountryDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    breadcrumbsItems: [{ label: 'Countries' }]
  });
  ...
  const [getCountryList] = useLazyQuery(COUNTRY_LIST, {
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/countries/edit/' + state.selectedRow);
    },
