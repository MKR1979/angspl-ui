...
import { TARGET_LIST, DELETE_TARGET } from '@/app/graphql/Target';
...
const useTargetList = ({ arrTargetDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrTargetDTO: arrTargetDTO,
    ...
    breadcrumbsItems: [{ label: 'Targets' }]
  });
  ...
  const [getTargetList] = useLazyQuery(TARGET_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getTargetList();
    if (!error && data?.getTargetList?.targets) {
      arrTargetDTO = data.getTargetList.targets.map((item: TargetDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getTargetList?.total_records) {
        total_records = data.getTargetList.total_records;
      }
    }
    setState({
      arrTargetDTO: arrTargetDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/targets/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/targets/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteTarget, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteTarget, getData, state.arrSelectedId, toggleDialog]
  );
