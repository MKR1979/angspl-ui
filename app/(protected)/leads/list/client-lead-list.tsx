...
const ClientLeadList = ({ arrLeadDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useLeadList({ arrLeadDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/leads/view/' + params.row.id}>{params.row.first_name}</MyLink>,
        []
      )
    },
    {
  ...
  console.log('hello', state.arrLeadDTO);
  return (
    <>
...
export default memo(ClientLeadList, (prevProps, nextProps) => {
