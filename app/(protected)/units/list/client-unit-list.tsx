...
const ClientUnitList = ({ arrUnitDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useUnitList({ arrUnitDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/units/view/' + params.row.id}>{params.row.unit_name}</MyLink>,
        []
      )
    },
    {
...
export default memo(ClientUnitList, (prevProps, nextProps) => {
