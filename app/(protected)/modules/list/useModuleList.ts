...
import { MODULE_LIST, DELETE_MODULE } from '@/app/graphql/Module';
...
const useModuleList = ({ arrModuleDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrModuleDTO: arrModuleDTO,
    ...
    breadcrumbsItems: [{ label: 'Modules' }]
  });
  ...
  const [getModuleList] = useLazyQuery(MODULE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getModuleList();
      if (!error && data) {
        arrModuleDTO = data.getModuleList.modules.map((item: ModuleDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getModuleList.total_records;
      }
      setState({
        arrModuleDTO: arrModuleDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/modules/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/modules/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteModule, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteModule, getData, state.arrSelectedId, toggleDialog]
  );
