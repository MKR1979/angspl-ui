...
import { GridEventListener, GridFilterModel, GridRowSelectionModel, GridSortModel, useGridApiRef, GridInitialState } from '@mui/x-data-grid';
...
import { DELETE_INTERVIEW_QUESTIONS, INTERVIEW_QUESTIONS_LIST } from '@/app/graphql/InterviewQuestions';
...
const useStudyNotesList = ({ arrInterviewQuestionsDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrInterviewQuestionsDTO: arrInterviewQuestionsDTO,
    ...
    breadcrumbsItems: [{ label: 'Interview Questions' }]
  });
  ...
  const [getInterviewQuestionsList] = useLazyQuery(INTERVIEW_QUESTIONS_LIST, {
  ...
  const [deleteInterviewQuestions] = useMutation(DELETE_INTERVIEW_QUESTIONS, {});
  ...
  const getInterviewQuestionsData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getInterviewQuestionsList();
      if (!error && data) {
        arrInterviewQuestionsDTO = data.getInterviewQuestionsList.studyNotes.map((item: InterviewQuestionsDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getInterviewQuestionsList.total_records;
      }
      setState({
        arrInterviewQuestionsDTO: arrInterviewQuestionsDTO,
        total_records: total_records,
  ...
  }, [getInterviewQuestionsData, state.filter_text, state.sort_field, state.sort_direction, paginationModel]);
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteInterviewQuestions({
          variables: {
            ids: params
          }
        });
    ...
    [deleteInterviewQuestions, getInterviewQuestionsData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteInterviewQuestions, getInterviewQuestionsData, state.arrSelectedId, toggleDialog]
  );
