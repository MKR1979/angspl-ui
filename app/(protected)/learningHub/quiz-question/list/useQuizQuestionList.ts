...
import { DELETE_QUIZ_QUESTION, QUIZ_QUESTION_LIST } from '@/app/graphql/QuizQuestion';
...
const useQuizQuestionList = ({ arrQuizQuestionDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrQuizQuestionDTO: arrQuizQuestionDTO,
    ...
    breadcrumbsItems: [{ label: 'Exam Question' }]
  });
  ...
  const [getQuizQuestionList] = useLazyQuery(QUIZ_QUESTION_LIST, {
  ...
  const [deleteQuizQuestion] = useMutation(DELETE_QUIZ_QUESTION, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getQuizQuestionList();
      if (!error && data) {
        arrQuizQuestionDTO = data.getQuizQuestionList.quizQuestions.map((item: QuizQuestionDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getQuizQuestionList.total_records;
      }
      setState({ arrQuizQuestionDTO: arrQuizQuestionDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
    } catch (err) {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/quiz-question/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/quiz-question/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteQuizQuestion, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/quiz-question/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteQuizQuestion, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useQuizQuestionList;
