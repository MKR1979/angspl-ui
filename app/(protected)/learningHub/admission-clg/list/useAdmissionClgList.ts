...
import { DELETE_ADMISSION_CLG, GET_ADMISSION_CLG_LIST } from '@/app/graphql/AdmissionClg';
...
type StateType = {
  isLoading: boolean;
  arrAdmissionClgDTO: AdmissionClgDTO[];
...
const useAdmissionClgList = ({ arrAdmissionClgDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrAdmissionClgDTO: arrAdmissionClgDTO,
  ...
  const [getAdmissionClgList] = useLazyQuery(GET_ADMISSION_CLG_LIST, {
  ...
  const [deleteAdmissionClg] = useMutation(DELETE_ADMISSION_CLG, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getAdmissionClgList();
      if (!error && data) {
        arrAdmissionClgDTO = data.getAdmissionClgList.admissionClgs.map((item: AdmissionClgDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getAdmissionClgList.total_records;
      }
      setState({
        arrAdmissionClgDTO: arrAdmissionClgDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (params) => {
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-clg/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      dispatch(setIsEditMode(true));
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-clg/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteAdmissionClg({
          variables: {
            ids: params
          }
        });
    ...
    [deleteAdmissionClg, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-clg/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteAdmissionClg, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useAdmissionClgList;
