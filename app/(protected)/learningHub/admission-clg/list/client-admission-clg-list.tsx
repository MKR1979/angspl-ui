...
import useAdmissionClgList from './useAdmissionClgList';
...
const ClientAdmissionClgList = ({ arrAdmissionClgDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useAdmissionClgList({ arrAdmissionClgDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 230) ? (
            <MyLink href={`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-clg/view/` + params.row.id}>{params.row.first_name}</MyLink>
          ) : (
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrAdmissionClgDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 227)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 228)}
          ...
          {(findPermission(userPermissions, 228) || findPermission(userPermissions, 229)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 229) && (
                <MyMenuItem onClick={onEditClick}>
                  <MyEditIcon />
                  Edit
                </MyMenuItem>
              )}
              {findPermission(userPermissions, 228) && (
...
export default memo(ClientAdmissionClgList, (prevProps, nextProps) => {
