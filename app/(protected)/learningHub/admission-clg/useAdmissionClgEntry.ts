...
import { ADD_ADMISSION_CLG, GET_ADMISSION_CLG, UPDATE_ADMISSION_CLG, GET_ADMISSION_CLG_EMAIL_EXIST } from '@/app/graphql/AdmissionClg';
...
const useAdmissionClgEntry = ({ dtoAdmissionClg }: Props) => {
  ...
  const [addAdmissionClg] = useMutation(ADD_ADMISSION_CLG, {});
  ...
  const [updateAdmissionClg] = useMutation(UPDATE_ADMISSION_CLG, {});
  ...
  const IsEMailExist = useCallback(async (): Promise<boolean> => {
    if (isEditMode && emailCopy.trim() === state.dtoAdmissionClg.email.trim()) {
      return false;
    }
    ...
    return exist;
  }, [getAdmissionClgEMailExist, state.dtoAdmissionClg.id, state.dtoAdmissionClg.email, isEditMode, emailCopy]);
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      setEmailCopy(data.getAdmissionClg.email);
      setState({ dtoAdmissionClg: dtoAdmissionClg } as StateType);
    } catch (err) {
      console.error('Error loading quiz question:', err);
      showSnackbar(gMessageConstants.SNACKBAR_DATA_FETCH_ERROR, 'error');
    }
  ...
  useEffect(() => {
    if (state.arrAdmissionStatusLookup.length > 0 && !state.dtoAdmissionClg.status) {
  ...
  const validateUgPassingYear = useCallback(async () => {
    const value = String(state.dtoAdmissionClg.ug_passing_year).trim();
    if (value === '') {
      return gMessageConstants.REQUIRED_FIELD;
    }
    return null;
  ...
  const validatePgPassingYear = useCallback(async () => {
    const value = String(state.dtoAdmissionClg.pg_passing_year).trim();
  ...
  const validateDiplomaPassingYear = useCallback(async () => {
    const value = String(state.dtoAdmissionClg.diploma_passing_year).trim();
  ...
  const onCourseNameChange = useCallback(
    async (event: any, value: unknown) => {
      setState({
        dtoAdmissionClg: {
          ...state.dtoAdmissionClg, course_id: (value as LookupDTO).id,
  ...
  //   if (state.dtoAdmissionClg.user_name.trim() === '') {
  //     return gMessageConstants.REQUIRED_FIELD;
  //   } else {
  //     return null;
  //   }
  // }, [state.dtoAdmissionClg.user_name]);
  ...
  const validateStatus = useCallback(async () => {
    if (state.dtoAdmissionClg.status.trim() === '') {
      return gMessageConstants.REQUIRED_FIELD;
    } else {
      return null;
    }
  }, [state.dtoAdmissionClg.status]);
  ...
  //   const highSchoolYear = (state.dtoAdmissionClg.high_school_year ?? '').toString().trim();
  //   const intermediateYear = (state.dtoAdmissionClg.intermediate_year ?? '').toString().trim();
  //   if (highSchoolYear === '') {
  //     return gMessageConstants.REQUIRED_FIELD;
  //   }
  //   if (intermediateYear !== '' && Number(highSchoolYear) >= Number(intermediateYear)) {
  //     return 'High School year should be less than Intermediate year';
  //   }
  //   return null;
  // }, [state.dtoAdmissionClg.high_school_year, state.dtoAdmissionClg.intermediate_year]);

  const validateXYear = useCallback(async () => {
    const highSchoolYear = (state.dtoAdmissionClg.high_school_year ?? '').toString().trim();
    const intermediateYear = (state.dtoAdmissionClg.intermediate_year ?? '').toString().trim();
    const is10threq = state.dtoCourse.is10threq; // or wherever you store it
  ...
  //     return gMessageConstants.REQUIRED_FIELD;
  //   }
  //   if (highSchoolYear !== '' && Number(intermediateYear) <= Number(highSchoolYear)) {
  ...
  // helper function
  // const normalizeValue = (val: string | number | Date | File | null | undefined) => {
  //   if (typeof val === 'string') {
  //     return val.trim(); // safe trim
  //   }
  //   if (val == null) {
  //     return ''; // null or undefined → empty string
  //   }
  //   return String(val); // convert numbers, Date, File to string for validation
  // };

  // const validateRequiredField = useCallback(
  //   async (fieldName: keyof typeof state.dtoAdmissionClg) => {
  //     const value = normalizeValue(state.dtoAdmissionClg[fieldName]);
  //     if (value === '') {
  //       return gMessageConstants.REQUIRED_FIELD;
  //     }
  //     return null;
  ...
  // const onRequiredFieldBlur = useCallback(
  //   async (fieldName: keyof typeof state.dtoAdmissionClg) => {
  //     const error = await validateRequiredField(fieldName);
  //     setState({
  //       errorMessages: { ...state.errorMessages, [fieldName]: error }
  //     } as StateType);
  //   },
  //   [validateRequiredField, state.errorMessages]
  // );
  ...
  const onRollNoChange = (fieldName: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    const digitsOnly = e.target.value.replace(/\D/g, '');
    if (digitsOnly.length <= gConstants.ROLL_NO_LENGTH) {
      onInputChange({
        ...e,
        target: { ...e.target, value: digitsOnly, name: fieldName }
      });
  ...
  const onCgpaChange = (fieldName: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Convert to number and clamp max 10
    const numVal = parseFloat(value);
  ...
  //   const newErrors: ErrorMessageType = { ...ERROR_MESSAGES };
  //   const requiredFields: (keyof typeof state.dtoAdmissionClg)[] = [
  //     'first_name',
  //     'course_type',
  //     'last_name',
  //     'email',
  ...
  //     const error = await validateRequiredField(field);
  ...
  //   setState({ errorMessages: newErrors } as StateType);
  //   return isFormValid;
  // }, [ERROR_MESSAGES, validateRequiredField]);

  //   const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  //      const checked = event.target.checked;
  // setIsSameAddress(checked);
  //     if(checked){
  //       state.dtoAdmissionClg.corr_address = state.dtoAdmissionClg.address,
  //       state.dtoAdmissionClg.corr_country_name = state.dtoAdmissionClg.country_name,
  //       state.dtoAdmissionClg.corr_country_id = state.dtoAdmissionClg.country_id,
  //       state.dtoAdmissionClg.corr_city_name = state.dtoAdmissionClg.city_name,
  //       state.dtoAdmissionClg.corr_zip_code = state.dtoAdmissionClg.zip_code,
  //     }else{
  ...
  const validateForm = useCallback(async () => {
    ...
    // errorMessages.father_email = await validateFEMailId();
    // if (errorMessages.father_email) {
    //   isFormValid = false;
    // }
    // errorMessages.mother_email = await validateMEMailId();
    // if (errorMessages.mother_email) {
    //   isFormValid = false;
    // }
    errorMessages.father_phone_no = await validateFPhoneNo();
  ...
  const onSaveClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>) => {
      ...
      try {
        if (await validateForm()) {
          ...
          if (state.dtoAdmissionClg.id === 0) {
            const { data } = await addAdmissionClg({
              variables: {
                data: {
            ...
            const { data } = await updateAdmissionClg({
              variables: {
                id: state.dtoAdmissionClg.id,
                data: {
                  admission_date: state.dtoAdmissionClg.admission_date,
                  ...
                  family_samagra_id: state.dtoAdmissionClg.family_samagra_id,
                  student_pen_no: state.dtoAdmissionClg.student_pen_no,
                  status: state.dtoAdmissionClg.status
                },
                photo: state.dtoAdmissionClg.photo,
                aadhaar_card: state.dtoAdmissionClg.aadhaar_card,
                other_certificate: state.dtoAdmissionClg.other_certificate,
    ...
    [validateForm, addAdmissionClg, state.dtoAdmissionClg, router, updateAdmissionClg]
  );
...
export default useAdmissionClgEntry;
