...
import { DELETE_ADMISSION_SCH, GET_ADMISSION_SCH_LIST } from '@/app/graphql/AdmissionSch';
...
type StateType = {
  isLoading: boolean;
  arrAdmissionSchoolDTO: AdmissionSchoolDTO[];
...
const useAdmissionSchList = ({ arrAdmissionSchoolDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrAdmissionSchoolDTO: arrAdmissionSchoolDTO,
  ...
  const [getAdmissionSchList] = useLazyQuery(GET_ADMISSION_SCH_LIST, {
  ...
  const [deleteAdmissionSch] = useMutation(DELETE_ADMISSION_SCH, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getAdmissionSchList();
      if (!error && data) {
        arrAdmissionSchoolDTO = data.getAdmissionSchList.admissionSchs.map((item: AdmissionSchoolDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getAdmissionSchList.total_records;
      }
      setState({
        arrAdmissionSchoolDTO: arrAdmissionSchoolDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (params) => {
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-sch/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      dispatch(setIsEditMode(true));
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-sch/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteAdmissionSch({
          variables: {
            ids: params
          }
        });
    ...
    [deleteAdmissionSch, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/admission-sch/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteAdmissionSch, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useAdmissionSchList;
