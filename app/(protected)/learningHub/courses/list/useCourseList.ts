...
import { COURSE_LIST, DELETE_COURSE } from '@/app/graphql/Course';
...
const useCourseList = ({ arrCourseDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrCourseDTO: arrCourseDTO,
    ...
    breadcrumbsItems: [{ label: 'Courses' }]
  });
  ...
  const [getCourseList] = useLazyQuery(COURSE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      if (!error && data) {
        arrCourseDTO = data.getCourseList.courses.map((item: courseDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getCourseList.total_records;
      }
      setState({ arrCourseDTO: arrCourseDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
    } catch (err) {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (params) => {
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/courses/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/courses/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteCourse, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/courses/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteCourse, getData, state.arrSelectedId, toggleDialog]
  );
