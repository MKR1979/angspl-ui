...
import { COURSE_TYPE_LIST, DELETE_COURSE_TYPE } from '@/app/graphql/CourseType';
...
type StateType = {
  isLoading: boolean;
  arrCourseTypeDTO: CourseTypeDTO[];
...
const useCourseTypeList = ({ arrCourseTypeDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrCourseTypeDTO: arrCourseTypeDTO,
    ...
    breadcrumbsItems: [{ label: 'Course Types' }]
  });
  ...
  const [getCourseTypeList] = useLazyQuery(COURSE_TYPE_LIST, {
  ...
  const [deleteCourseType] = useMutation(DELETE_COURSE_TYPE, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getCourseTypeList();
      if (!error && data) {
        arrCourseTypeDTO = data.getCourseTypeList.course_types.map((item: CourseTypeDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getCourseTypeList.total_records;
      }
      setState({
        arrCourseTypeDTO: arrCourseTypeDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/course-types/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/course-types/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteCourseType({
          variables: {
            ids: params
          }
        });
    ...
    [deleteCourseType, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/course-types/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteCourseType, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useCourseTypeList;
