...
import useCourseTypeList from './useCourseTypeList';
...
const ClientCourseTypeList = ({ arrCourseTypeDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useCourseTypeList({ arrCourseTypeDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      headerName: 'Course Type Name',
      flex: 1,
      minWidth: 150,
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 89) ? (
            <MyLink href={`/${gConstants.ADMIN_STUDENT_DASHBOARD}/course-types/view/` + params.row.id}>{params.row.course_type_name}</MyLink>
          ) : (
            <span>{params.row.course_type_name}</span>
          ),
        [userPermissions]
      )
    },
    ...
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm CourseType Removal"
...
export default memo(ClientCourseTypeList, (prevProps, nextProps) => {
