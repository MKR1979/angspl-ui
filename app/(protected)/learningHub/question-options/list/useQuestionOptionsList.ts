...
import { QUESTION_OPTIONS_LIST, DELETE_QUESTION_OPTIONS } from '@/app/graphql/QuestionOptions';
...
const useQuestionOptionsList = ({ arrQuestionOptionsDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrQuestionOptionsDTO: arrQuestionOptionsDTO,
    ...
    breadcrumbsItems: [{ label: 'Question Options' }]
  });
  ...
  const [getQuestionOptionsList] = useLazyQuery(QUESTION_OPTIONS_LIST, {
  ...
  const [deleteQuestionOptions] = useMutation(DELETE_QUESTION_OPTIONS, {});
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getQuestionOptionsList();
      if (!error && data) {
        arrQuestionOptionsDTO = data.getQuestionOptionsList.questionOptions.map((item: QuestionOptionsDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getQuestionOptionsList.total_records;
      }
      setState({ arrQuestionOptionsDTO: arrQuestionOptionsDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
    } catch (err) {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/question-options/edit/` + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/question-options/edit/` + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteQuestionOptions({
          variables: {
            ids: params
          }
        });
    ...
    [deleteQuestionOptions, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push(`/${gConstants.ADMIN_STUDENT_DASHBOARD}/question-options/add`);
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteQuestionOptions, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useQuestionOptionsList;
