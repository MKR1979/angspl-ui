...
const useStateList = ({ arrStateDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrStateDTO: arrStateDTO,
  ...
  const [getStateList] = useLazyQuery(STATE_LIST, {
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/states/edit/' + state.selectedRow);
    },
