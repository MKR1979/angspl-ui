...
import { DOCUMENT_CATEGORY_LIST, DELETE_DOCUMENT_CATEGORY } from '@/app/graphql/DocumentCategory';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrDocumentCategoryDTO: DocumentCategoryDTO[];
...
const useDocumentCategoryList = ({ arrDocumentCategoryDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrDocumentCategoryDTO: arrDocumentCategoryDTO,
    ...
    breadcrumbsItems: [{ label: 'Document Categories' }]
  });
  ...
  const [getDocumentCategoryList] = useLazyQuery(DOCUMENT_CATEGORY_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getDocumentCategoryList();
    if (!error && data?.getDocumentCategoryList?.documentCategories) {
      arrDocumentCategoryDTO = data.getDocumentCategoryList.documentCategories.map((item: DocumentCategoryDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getDocumentCategoryList?.total_records) {
        total_records = data.getDocumentCategoryList.total_records;
      }
    }
    setState({
      arrDocumentCategoryDTO: arrDocumentCategoryDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/document-categories/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-categories/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteDocumentCategory({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteDocumentCategory) {
          getData();
    ...
    [deleteDocumentCategory, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-categories/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDocumentCategory, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useDocumentCategoryList;
