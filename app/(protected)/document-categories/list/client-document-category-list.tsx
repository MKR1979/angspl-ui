...
import useDocumentCategoryList from './useDocumentCategoryList';
...
const ClientDocumentCategoryList = ({ arrDocumentCategoryDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useDocumentCategoryList({ arrDocumentCategoryDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => (
          <MyLink href={'/document-categories/view/' + params.row.id}>{params.row.document_category_name}</MyLink>
        ),
        []
      )
    }
    ...
    <>
      ...
      <MyCard>
        <MyCardContent>
          <MyDataGrid
            ...
            rows={state.arrDocumentCategoryDTO}
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Document Category Removal"
...
export default memo(ClientDocumentCategoryList, (prevProps, nextProps) => {
