...
import { INDUSTRY_LIST, DELETE_INDUSTRY } from '@/app/graphql/Industry';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrIndustryDTO: IndustryDTO[];
...
const useIndustryList = ({ arrIndustryDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrIndustryDTO: arrIndustryDTO,
    ...
    breadcrumbsItems: [{ label: 'Industries' }]
  });
  ...
  const [getIndustryList] = useLazyQuery(INDUSTRY_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getIndustryList();
    if (!error && data?.getIndustryList?.industries) {
      arrIndustryDTO = data.getIndustryList.industries.map((item: IndustryDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getIndustryList?.total_records) {
        total_records = data.getIndustryList.total_records;
      }
    }
    setState({
      arrIndustryDTO: arrIndustryDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/industries/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/industries/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteIndustry({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteIndustry) {
          getData();
    ...
    [deleteIndustry, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteIndustry, getData, state.arrSelectedId, toggleDialog]
  );
