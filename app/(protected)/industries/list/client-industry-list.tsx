...
const ClientIndustryList = ({ arrIndustryDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useIndustryList({ arrIndustryDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/industries/view/' + params.row.id}>{params.row.industry_name}</MyLink>,
        []
      )
    }
  ];
    ...
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Industry Removal"
...
export default memo(ClientIndustryList, (prevProps, nextProps) => {
