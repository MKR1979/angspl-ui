...
const ClientMeetingList = ({ arrMeetingDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onDeleteSingleClose,
    onCalendarViewClick
  } = useMeetingList({ arrMeetingDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/meetings/view/' + params.row.id}>{params.row.subject}</MyLink>,
        []
      )
    },
    {
  ...
  console.log('hello', state.arrMeetingDTO);
  return (
    <>
      <MyBreadcrumbs
        items={state.breadcrumbsItems}
        secondary={
          <MyButton variant="outlined" sx={{ backgroundColor: '#fff', color: '#000' }} onClick={onCalendarViewClick}>
            Calendar View
          </MyButton>
        }
      ></MyBreadcrumbs>
