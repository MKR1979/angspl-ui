...
import { MEETING_LIST, DELETE_MEETING } from '@/app/graphql/Meeting';
...
const useMeetingList = ({ arrMeetingDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrMeetingDTO: arrMeetingDTO,
  ...
  const [getMeetingList] = useLazyQuery(MEETING_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getMeetingList();
      if (!error && data?.getMeetingList?.meetings) {
        arrMeetingDTO = data.getMeetingList.meetings.map((item: MeetingDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        if (data?.getMeetingList?.total_records) {
          total_records = data.getMeetingList.total_records;
        }
      }
      setState({
        arrMeetingDTO: arrMeetingDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/meetings/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/meetings/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteMeeting, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteMeeting, getData, state.arrSelectedId, toggleDialog]
  );
  ...
  const onCalendarViewClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>) => {
      event.preventDefault();
      router.push('/meetings/list/calendar');
    },
    [router]
  );
