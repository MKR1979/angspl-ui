...
const useCurrencyList = ({ arrCurrencyDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    breadcrumbsItems: [{ label: 'Currencies' }]
  });
  ...
  const [getCurrencyList] = useLazyQuery(CURRENCY_LIST, {
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/currencies/edit/' + state.selectedRow);
    },
