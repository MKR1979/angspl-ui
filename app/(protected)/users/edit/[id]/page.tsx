...
export default async function EditUserPage({ params }: Props) {
  ...
  try {
    ...
    if (results[2]?.data?.getTypeLookup) {
      arrTypeLookup = results[2].data.getTypeLookup;
    }
  } catch (error) {
    console.error('[ERROR] Failed to load user or role data:', error);
  }

  return <ClientEditUser dtoUser={dtoUser} arrRoleLookup={arrRoleLookup} arrTypeLookup={arrTypeLookup} />;
}
