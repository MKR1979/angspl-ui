...
import { ACCOUNT_TYPE_LIST, DELETE_ACCOUNT_TYPE } from '@/app/graphql/AccountType';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrAccountTypeDTO: AccountTypeDTO[];
...
const useAccountTypeList = ({ arrAccountTypeDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrAccountTypeDTO: arrAccountTypeDTO,
    ...
    breadcrumbsItems: [{ label: 'Account Types' }]
  });
  ...
  const [getAccountTypeList] = useLazyQuery(ACCOUNT_TYPE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getAccountTypeList();
    if (!error && data?.getAccountTypeList.accountTypes) {
      arrAccountTypeDTO = data.getAccountTypeList.accountTypes.map((item: AccountTypeDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getAccountTypeList?.total_records) {
        total_records = data.getAccountTypeList.total_records;
      }
    }
    setState({
      arrAccountTypeDTO: arrAccountTypeDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/account-types/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/account-types/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteAccountType({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteAccountType) {
    ...
    [deleteAccountType, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteAccountType, getData, state.arrSelectedId, toggleDialog]
  );
