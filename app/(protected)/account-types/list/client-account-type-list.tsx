...
import useAccountTypeList from './useAccountTypeList';
...
const ClientAccountTypeList = ({ arrAccountTypeDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useAccountTypeList({ arrAccountTypeDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/account-types/view/' + params.row.id}>{params.row.account_type_name}</MyLink>,
        []
      )
    }
  ];
    ...
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Account Type Removal"
...
export default memo(ClientAccountTypeList, (prevProps, nextProps) => {
