...
import useIncotermList from './useIncotermList';
...
const ClientIncotermList = ({ arrIncotermDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useIncotermList({ arrIncotermDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'incoterm_code',
      headerName: 'Incoterm Code',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/incoterms/view/' + params.row.id}>{params.row.incoterm_code}</MyLink>,
        []
      )
    },
    {
      field: 'incoterm_description',
      headerName: 'Incoterm Description',
      flex: 1
    }
  ];
    ...
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Incoterm Removal"
...
export default memo(ClientIncotermList, (prevProps, nextProps) => {
