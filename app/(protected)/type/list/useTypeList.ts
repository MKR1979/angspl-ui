...
import { TYPE_LIST, DELETE_TYPE } from '@/app/graphql/Type';
...
const useTypeList = ({ arrTypeDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrTypeDTO: arrTypeDTO,
  ...
  const [getTypeList] = useLazyQuery(TYPE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getTypeList();
      if (!error && data) {
        arrTypeDTO = data.getTypeList.types.map((item: TypeDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getTypeList.total_records;
      }
      setState({
        arrTypeDTO: arrTypeDTO,
        total_records: total_records,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/type/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/type/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteType, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteType, getData, state.arrSelectedId, toggleDialog]
  );
