...
const ClientTypeList = ({ arrTypeDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useTypeList({ arrTypeDTO, total_records });
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 174) ? (
            <MyLink href={'/type/view/' + params.row.id}>{params.row.type_name}</MyLink>
          ) : (
...
export default memo(ClientTypeList, (prevProps, nextProps) => {
