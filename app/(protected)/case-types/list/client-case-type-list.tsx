...
import useCaseTypeList from './useCaseTypeList';
...
const ClientCaseTypeList = ({ arrCaseTypeDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useCaseTypeList({ arrCaseTypeDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      headerName: 'Case Type Name',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/case-types/view/' + params.row.id}>{params.row.case_type_name}</MyLink>,
        []
      )
    }
  ];
  console.log('hello', state.arrCaseTypeDTO);
  return (
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Case Type Removal"
...
export default memo(ClientCaseTypeList, (prevProps, nextProps) => {
