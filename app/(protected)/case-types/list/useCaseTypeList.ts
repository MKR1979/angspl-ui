...
import { CASE_TYPE_LIST, DELETE_CASE_TYPE } from '@/app/graphql/CaseType';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrCaseTypeDTO: CaseTypeDTO[];
...
const useCaseTypeList = ({ arrCaseTypeDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrCaseTypeDTO: arrCaseTypeDTO,
    ...
    breadcrumbsItems: [{ label: 'Case Types' }]
  });
  ...
  const [getCaseTypeList] = useLazyQuery(CASE_TYPE_LIST, {
  ...
  const [deleteCaseType] = useMutation(DELETE_CASE_TYPE, {});

  const getData = useCallback(async (): Promise<void> => {
    setState({ isLoading: true } as StateType);
    let arrCaseTypeDTO: CaseTypeDTO[] = [];
    let total_records: number = 0;
    const { error, data } = await getCaseTypeList();
    if (!error && data?.getCaseTypeList?.caseTypes) {
      arrCaseTypeDTO = data.getCaseTypeList.caseTypes.map((item: CaseTypeDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getCaseTypeList?.total_records) {
        total_records = data.getCaseTypeList.total_records;
      }
    }
    setState({
      arrCaseTypeDTO: arrCaseTypeDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/case-types/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/case-types/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteCaseType({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteCaseType) {
          getData();
    ...
    [deleteCaseType, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteCaseType, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useCaseTypeList;
