...
import { OPPORTUNITY_LIST, DELETE_OPPORTUNITY } from '@/app/graphql/Opportunity';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrOpportunityDTO: OpportunityDTO[];
...
const useOpportunityList = ({ arrOpportunityDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrOpportunityDTO: arrOpportunityDTO,
    ...
    breadcrumbsItems: [{ label: 'Opportunities' }]
  });
  ...
  const [getOpportunityList] = useLazyQuery(OPPORTUNITY_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getOpportunityList();
    if (!error && data?.getOpportunityList?.opportunities) {
      arrOpportunityDTO = data.getOpportunityList.opportunities.map((item: OpportunityDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getOpportunityList?.total_records) {
        total_records = data.getOpportunityList.total_records;
      }
    }
    setState({
      arrOpportunityDTO: arrOpportunityDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/opportunities/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/opportunities/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteOpportunity, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteOpportunity, getData, state.arrSelectedId, toggleDialog]
  );
