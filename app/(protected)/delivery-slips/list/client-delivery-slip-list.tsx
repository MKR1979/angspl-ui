...
import useDeliverySlipList from './useDeliverySlipList';
...
const ClientDeliverySlipList = ({ arrDeliverySlipDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useDeliverySlipList({ arrDeliverySlipDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'delivery_slip_no',
      headerName: 'Delivery Slip #',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/delivery-slips/view/' + params.row.id}>{params.row.delivery_slip_no}</MyLink>,
        []
      )
    },
    {
      field: 'delivery_slip_date',
      headerName: 'Delivery Slip Date',
      flex: 1,
  ...
  console.log('hello', state.arrDeliverySlipDTO);
  return (
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Delivery Slip Removal"
...
export default memo(ClientDeliverySlipList, (prevProps, nextProps) => {
