...
import { DELIVERY_SLIP_LIST, DELETE_DELIVERY_SLIP } from '@/app/graphql/DeliverySlip';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrDeliverySlipDTO: DeliverySlipDTO[];
...
const useDeliverySlipList = ({ arrDeliverySlipDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrDeliverySlipDTO: arrDeliverySlipDTO,
    ...
    breadcrumbsItems: [{ label: 'Delivery Slips' }]
  });
  ...
  const [getDeliverySlipList] = useLazyQuery(DELIVERY_SLIP_LIST, {
  ...
  const [deleteDeliverySlip] = useMutation(DELETE_DELIVERY_SLIP, {});

  const getData = useCallback(async (): Promise<void> => {
    setState({ isLoading: true } as StateType);
    let arrDeliverySlipDTO: DeliverySlipDTO[] = [];
    let total_records: number = 0;
    const { error, data } = await getDeliverySlipList();
    if (!error && data?.getDeliverySlipList?.deliverySlips) {
      arrDeliverySlipDTO = data.getDeliverySlipList.deliverySlips.map((item: DeliverySlipDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getDeliverySlipList?.total_records) {
        total_records = data.getDeliverySlipList.total_records;
      }
    }
    setState({
      arrDeliverySlipDTO: arrDeliverySlipDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/delivery-slips/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/delivery-slips/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteDeliverySlip({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteDeliverySlip) {
          getData();
    ...
    [deleteDeliverySlip, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/delivery-slips/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDeliverySlip, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useDeliverySlipList;
