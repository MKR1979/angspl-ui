...
import { DISTRICT_LIST, DELETE_DISTRICT } from '@/app/graphql/District';
...
const useDistrictList = ({ arrDistrictDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrDistrictDTO: arrDistrictDTO,
    ...
    breadcrumbsItems: [{ label: 'Districts' }]
  });
  ...
  const [getDistrictList] = useLazyQuery(DISTRICT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getDistrictList();
      if (!error && data) {
        arrDistrictDTO = data.getDistrictList.districts.map((item: DistrictDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        total_records = data.getDistrictList.total_records;
      }
      setState({ arrDistrictDTO: arrDistrictDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
    } catch (err) {
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/districts/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/districts/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteDistrict({
          variables: {
            ids: params
          }
        });
    ...
    [deleteDistrict, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDistrict, getData, state.arrSelectedId, toggleDialog]
  );
