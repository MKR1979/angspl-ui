...
import useStateList from './useDistrictList';
...
const ClientDistrictList = ({ arrDistrictDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useStateList({ arrDistrictDTO, total_records });
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) =>
          findPermission(userPermissions, 159) ? (
            <MyLink href={'/districts/view/' + params.row.id}>{params.row.district_name}</MyLink>
          ) : (
            <span>{params.row.district_name}</span>
          ),
        [userPermissions]
      )
    },
        ...
        <MyCardContent sx={{ overflowX: 'auto' }}>
          <div style={{ minWidth: `${columns.length * 150}px` }}>
            <MyDataGrid
              ...
              rows={state.arrDistrictDTO}
              rowCount={state.total_records}
              columns={columns}
              loading={state.isLoading}
              handleContextMenu={handleContextMenu}
              onAddClick={onAddClick}
              showAddButton={findPermission(userPermissions, 156)}
              onDeleteClick={onDeleteAllClick}
              showDeleteButton={state.arrSelectedId.length > 0 && findPermission(userPermissions, 157)}
          ...
          {(findPermission(userPermissions, 157) || findPermission(userPermissions, 158)) && (
            <MyMenu
            ...
            >
              {findPermission(userPermissions, 158) && (
              ...
              {findPermission(userPermissions, 157) && (
...
export default memo(ClientDistrictList, (prevProps, nextProps) => {
