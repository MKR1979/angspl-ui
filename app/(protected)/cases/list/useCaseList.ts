...
import { CASE_LIST, DELETE_CASE } from '@/app/graphql/Case';
...
const useCaseList = ({ arrCaseDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrCaseDTO: arrCaseDTO,
  ...
  const [getCaseList] = useLazyQuery(CASE_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    if (!error && data?.getCaseList?.cases) {
      arrCaseDTO = data.getCaseList.cases.map((item: CaseDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getCaseList?.total_records) {
        total_records = data.getCaseList.total_records;
      }
    }
    setState({
      arrCaseDTO: arrCaseDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (params) => {
      router.push('/cases/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/cases/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteCase, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteCase, getData, state.arrSelectedId, toggleDialog]
  );
