...
const ClientRoleList = ({ arrRoleDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useRoleList({ arrRoleDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/roles/view/' + params.row.id}>{params.row.role_name}</MyLink>,
        []
      )
    }
  ];
...
export default memo(ClientRoleList, (prevProps, nextProps) => {
