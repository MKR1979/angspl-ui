...
import { CONTACT_LIST, DELETE_CONTACT } from '@/app/graphql/Contact';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrContactDTO: ContactDTO[];
...
const useContactList = ({ arrContactDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrContactDTO: arrContactDTO,
    ...
    breadcrumbsItems: [{ label: 'Contacts' }]
  });
  ...
  const [getContactList] = useLazyQuery(CONTACT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    ...
    const { error, data } = await getContactList();
    if (!error && data?.getContactList?.contacts) {
      arrContactDTO = data.getContactList.contacts.map((item: ContactDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getContactList?.total_records) {
        total_records = data.getContactList.total_records;
      }
    }
    setState({
      arrContactDTO: arrContactDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/contacts/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/contacts/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteContact, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteContact, getData, state.arrSelectedId, toggleDialog]
  );
