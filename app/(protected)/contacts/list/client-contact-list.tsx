...
const ClientContactList = ({ arrContactDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useContactList({ arrContactDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/contacts/view/' + params.row.id}>{params.row.first_name}</MyLink>,
        []
      )
    },
    {
  ...
  console.log('hello', state.arrContactDTO);
  return (
    <>
...
export default memo(ClientContactList, (prevProps, nextProps) => {
