...
import { DELETE_CODE_PROJECT, CODE_PROJECT_LIST } from '@/app/graphql/CodeProject';
...
const useCodeProjectList = ({ arrCodeProjectDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrCodeProjectDTO: arrCodeProjectDTO,
    ...
    breadcrumbsItems: [{ label: 'Code Project' }]
  });
  ...
  const [getCodeProjectList] = useLazyQuery(CODE_PROJECT_LIST, {
  ...
  const [deleteCodeProject] = useMutation(DELETE_CODE_PROJECT, {});

  const getData =useCallback(async (): Promise<void> => {
    setState({ isLoading: true } as StateType);
    let arrCodeProjectDTO: CodeProjectDTO[] = [];
    let total_records: number = 0;
    const { error, data } = await getCodeProjectList();
    if (!error && data) {
      arrCodeProjectDTO = data.getCodeProjectList.codeProjects.map((item: CodeProjectDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      total_records = data.getCodeProjectList.total_records;
    }
    setState({ arrCodeProjectDTO: arrCodeProjectDTO, total_records: total_records, isLoading: false, arrSelectedId: [] as string[] } as StateType);
  }, [getCodeProjectList]);
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> =useCallback(
    async (
      ...
      router.push('/code-projects/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick =useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/code-projects/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle =useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      ...
      const { data } = await deleteCodeProject({
        variables: {
          ids: params
        }
      });
    ...
    [deleteCodeProject, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected =useCallback(
    ...
    [deleteCodeProject, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useCodeProjectList;