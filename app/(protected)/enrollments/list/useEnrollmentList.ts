...
import { ENROLLMENT_LIST, DELETE_ENROLLMENT } from '@/app/graphql/Enrollment';
...
const useEnrollmentList = ({ arrEnrollmentDTO, total_records }: Props) => {
  const router = useRouter();
  const apiRef = useGridApiRef();
  const INITIAL_STATE: StateType = Object.freeze({
    isLoading: false,
    arrEnrollmentDTO: arrEnrollmentDTO,
    ...
    breadcrumbsItems: [{ label: 'Enrollments' }]
  });
  ...
  const [getEnrollmentList] = useLazyQuery(ENROLLMENT_LIST, {
  ...
  const getData = useCallback(async (): Promise<void> => {
    try {
      ...
      const { error, data } = await getEnrollmentList();
      if (!error && data?.getEnrollmentList?.enrollments) {
        arrEnrollmentDTO = data.getEnrollmentList.enrollments.map((item: EnrollmentDTO) => {
          return { ...item, id: parseInt(item.id.toString()) };
        });
        if (data?.getEnrollmentList?.total_records) {
          total_records = data.getEnrollmentList.total_records;
        }
      }
      setState({
        arrEnrollmentDTO: arrEnrollmentDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/enrollments/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/enrollments/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    ...
    [deleteEnrollment, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteEnrollment, getData, state.arrSelectedId, toggleDialog]
  );
  ...
  // const onCalendarViewClick = useCallback(
  //   async (event: React.MouseEvent<HTMLElement>) => {
  //     event.preventDefault();
  //     router.push('/enrollments/list/calendar');
  //   },
  //   [router]
  // );
...
export default useEnrollmentList;
