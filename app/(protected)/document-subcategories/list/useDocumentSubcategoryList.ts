...
import { DOCUMENT_SUBCATEGORY_LIST, DELETE_DOCUMENT_SUBCATEGORY } from '@/app/graphql/DocumentSubcategory';

type visibleDialog1Type = { id: number; visibility: boolean };

type StateType = {
  isLoading: boolean;
  arrDocumentSubcategoryDTO: DocumentSubcategoryDTO[];
...
const useDocumentSubcategoryList = ({ arrDocumentSubcategoryDTO, total_records }: Props) => {
  ...
  const INITIAL_STATE: StateType = Object.freeze({
    ...
    breadcrumbsItems: [{ label: 'Document Subcategories' }]
  });
  ...
  const [getDocumentSubcategoryList] = useLazyQuery(DOCUMENT_SUBCATEGORY_LIST, {
  ...
  const [deleteDocumentSubcategory] = useMutation(DELETE_DOCUMENT_SUBCATEGORY, {});

  const getData = useCallback(async (): Promise<void> => {
    setState({ isLoading: true } as StateType);
    let arrDocumentSubcategoryDTO: DocumentSubcategoryDTO[] = [];
    let total_records: number = 0;
    const { error, data } = await getDocumentSubcategoryList();
    if (!error && data?.getDocumentSubcategoryList?.documentSubcategories) {
      arrDocumentSubcategoryDTO = data.getDocumentSubcategoryList.documentSubcategories.map((item: DocumentSubcategoryDTO) => {
        return { ...item, id: parseInt(item.id.toString()) };
      });
      if (data?.getDocumentSubcategoryList?.total_records) {
        total_records = data.getDocumentSubcategoryList.total_records;
      }
    }
    setState({
      arrDocumentSubcategoryDTO: arrDocumentSubcategoryDTO,
  ...
  const onRowDoubleClick: GridEventListener<'rowDoubleClick'> = useCallback(
    async (
      ...
      router.push('/document-subcategories/edit/' + params.row.id);
    },
    [router]
  );
  ...
  const onEditClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-subcategories/edit/' + state.selectedRow);
    },
  ...
  const DeleteSingle = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      try {
        ...
        const { data } = await deleteDocumentSubcategory({
          variables: {
            ids: params
          }
        });
        await toggleDialog1(0);
        if (data?.deleteDocumentSubcategory) {
          getData();
    ...
    [deleteDocumentSubcategory, getData, state.visibleDialog1.id, toggleDialog1]
  );
  ...
  const onAddClick = useCallback(
    async (event: React.MouseEvent<HTMLElement>): Promise<void> => {
      event.preventDefault();
      router.push('/document-subcategories/add');
    },
    [router]
  );
  ...
  const DeleteSelected = useCallback(
    ...
    [deleteDocumentSubcategory, getData, state.arrSelectedId, toggleDialog]
  );
...
export default useDocumentSubcategoryList;
