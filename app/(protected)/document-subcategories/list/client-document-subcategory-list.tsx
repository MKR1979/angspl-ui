...
import useDocumentSubcategoryList from './useDocumentSubcategoryList';
...
const ClientDocumentSubcategoryList = ({ arrDocumentSubcategoryDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useDocumentSubcategoryList({ arrDocumentSubcategoryDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: 'document_subcategory_name',
      headerName: 'Document Subcategory Name',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => (
          <MyLink href={'/document-subcategories/view/' + params.row.id}>{params.row.document_subcategory_name}</MyLink>
        ),
        []
      )
    },
    ...
    <>
      ...
      <MyCard>
        <MyCardContent>
          <MyDataGrid
            ...
            rows={state.arrDocumentSubcategoryDTO}
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Document Subcategory Removal"
...
export default memo(ClientDocumentSubcategoryList, (prevProps, nextProps) => {
