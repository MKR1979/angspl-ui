...
import useLeadSourceList from './useLeadSourceList';
...
const ClientLeadSourceList = ({ arrLeadSourceDTO, total_records }: Props) => {
  const {
    state,
    apiRef,
    paginationModel,
    ...
    onRowDoubleClick,
    onDeleteSingleClose
  } = useLeadSourceList({ arrLeadSourceDTO, total_records });
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      headerName: 'Lead Source Name',
      flex: 1,
      renderCell: useCallback(
        (params: GridRenderCellParams) => <MyLink href={'/lead-sources/view/' + params.row.id}>{params.row.lead_source_name}</MyLink>,
        []
      )
    }
  ];
  console.log('hello', state.arrLeadSourceDTO);
  return (
    <>
      ...
      {state.visibleDialog && (
        <MyConfirmDialog
          ...
          title="Confirm Lead Source Removal"
...
export default memo(ClientLeadSourceList, (prevProps, nextProps) => {
