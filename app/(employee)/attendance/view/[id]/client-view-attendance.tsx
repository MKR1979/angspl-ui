...
import useViewAttendance from './useViewAttendance';
...
const ClientViewAttendance = ({ dtoAttendance }: Props) => {
  const { state, onCancelClick } = useViewAttendance({ dtoAttendance });
              ...
              <MyBox sx={{ mb: 0 }}>
                ...
                <MyTypography variant="subtitle2">
                  {state.dtoAttendance.attendance_time
                    ? dayjs.utc(state.dtoAttendance.attendance_time).format('YYYY-MM-DD hh:mm A')
                    : 'N/A'}
                </MyTypography>
              </MyBox>
            </MyGrid>
