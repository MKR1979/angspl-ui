'use client';
import '../(employee)/attendance/sentry.client.config'; // 👈 make sure path is correct
...
export default function RootLayout({
  ...
  const handleMenuChangePassword = useCallback(async () => {
    router.push('/change-emp-password');
    setAnchorEl(null);
    handleMobileMenuClose();
  }, [router, handleMobileMenuClose]);
  ...
  if (Array.isArray(siteConfig)) {
    siteConfig.forEach((config) => {
      switch (config.key) {
        case 'ENABLE_EMP_DASHBOARD':
          menuFlags.empDashboard = config.value === 'true';
          break;
        case 'ENABLE_ATTENDANCE':
          menuFlags.attendance = config.value === 'true';
          break;
                ...
                */}
                  {menuFlags.attendance && (
                    <CustomTreeItem
                      ...
                      onClick={() => {
                        toggleDrawer1();
                        if (findPermission(userPermissions, 15)) {
